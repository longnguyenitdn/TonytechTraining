...
const NOTI_NOT_FOUND =
  "Nothing to show! Please use add new func or clear searching info. Thank you!";
...
const displayPersonList = () => {
         ...
         <tr>
            ...
            <th scope="col">
               <div id="name_arrow" class=" hover-arrow">
                  <p id="name_arrow_direction" class="hide arrow-absolute" ><i class=" ${
                    fieldSort === "name"
                      ? statusSort === "up"
                        ? ARROW_DIRECTION_UP
            ...
            <th  scope="col">
               <div id="email_arrow" class=" hover-arrow">
                  <p id="email_arrow_direction" class="hide arrow-absolute"><i class=" ${
                    fieldSort === "email"
                      ? statusSort === "up"
                        ? ARROW_DIRECTION_UP
                        : ARROW_DIRECTION_DOWN
            ...
            <th  scope="col">
               <div id="phone_arrow" class=" hover-arrow">
                  <p id="phone_arrow_direction" class="hide arrow-absolute"><i class=" ${
                    fieldSort === "phone"
                      ? statusSort === "up"
                        ? ARROW_DIRECTION_UP
                        : ARROW_DIRECTION_DOWN
  ...
  for (
    let i = startPage;
    i < (endPage < list.length ? endPage : list.length);
    i++
  ) {
    tableString += `<tr>
         <td class="check-box">
         ...
         <td scope="row" class="text-left"><img class="img_border" src="${
           list[i].photo instanceof File
             ? URL.createObjectURL(list[i].photo)
             : "https://image.shutterstock.com/image-vector/fail-stamp-square-grungy-red-260nw-1726293580.jpg"
         }" alt="1"></td>
         <td>${list[i].name}</td>
         <td>${list[i].email}</td>
         <td>${list[i].phone ? list[i].phone : ""}</td>
         <td><i onclick="onclickToEdit(${
           list[i].id
         })" class="bi bi-pencil-square pencil"></i> </td>
         <td class="remove-wrap"><i class="bi bi-trash trash" onclick="openRemoveConfirm(${
           list[i].id
         })"></i>
         </td>
         </tr>`;
  ...
  document
    .getElementById("name_arrow")
    .addEventListener("click", sortListByName);
  document
    .getElementById("email_arrow")
    .addEventListener("click", sortListByEmail);
  document
    .getElementById("phone_arrow")
    .addEventListener("click", sortListByPhone);
  displayTotalCounter();
  document.querySelectorAll(".form-check-input").forEach((node) => {
    node.addEventListener("change", setIdCheckBox);
  });
  document
    .getElementById("select_all_checked")
    .addEventListener("change", handleAllCheckBoxStatus);
...
const displayTotalCounter = () => {
  ...
  let counter = inputPersons.reduce(
    (next, key) => {
      if (key.email !== "") {
        next.countEmail += 1;
      }
  ...
  document.getElementById("hasEmail").innerHTML =
    "Email: " + counter.countEmail;

  document.getElementById("hasPhone").innerHTML =
    "Phone: " + counter.countPhone;
...
const getPersonListWithCondition = () => {
  ...
  if (keySearch == "") {
    ...
    const checkAll = (obj) => {
      if (
        obj.name.includes(keySearch) ||
        obj.email.includes(keySearch) ||
        "obj.phone".includes(keySearch)
      ) {
        return true;
      } else {
        return false;
...
const handleAllCheckBoxStatus = (e) => {
  ...
  let checkList = inputList.slice(
    startPage,
    endPage < inputList.length ? endPage : inputList.length
  );
  if (e.target.checked) {
...
const renderPageNumber = (list) => {
  ...
  for (let i = 1; i <= totalPage; i++) {
    document.getElementById("pagination").innerHTML += `<li class=" ${
      currrentPage === i ? "checked-pagination pagination" : "pagination"
    }" id="page_${i}">${i}
      </li>`;
  }
};
...
document
  ...
  .addEventListener("keyup", debounce(handleSearch, 1500));
document.getElementById("addNewBtn").addEventListener("click", openModal);
document
  .getElementById("btn_show_checked")
  .addEventListener("click", removeListChecked);
displayPersonList();
