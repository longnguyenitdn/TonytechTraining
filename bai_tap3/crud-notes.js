    ...
    myFetch(link, option, note)
      .then((data) => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            document
              .getElementById(`note${notes[0].id}`)
...
const onclickToEdit = (e) => {
  ...
  document.getElementById("detail_edit_note_content").innerHTML =
    displayDetailNoteModal();
  document
    .getElementById("close_detail_modal")
    .addEventListener("click", closeDetailModal);
  document
    .getElementById("detail_edit_note_wrap")
    .addEventListener("click", handleEditNote);
};
...
const displayHandleLabelModal = () => {
  ...
  document.getElementById("note_option_label").innerHTML =
    stringHandleLabelModal;
...
const handleAddNewLabeltoNote = (id) => {
  ...
  let editNote = notes.find(
    (item) => item.id === parseInt(document.getElementById("optionId").value)
  );
...
const handleCheckBoxAll = (id) => {
  ...
  if (isExist) {
    ...
    document
      .getElementById(`note_wrap${checkAllId}`)
      .classList.remove("border-checkbox");
  } else {
    checkboxListId.push(checkAllId);
    document
      .getElementById(`note_wrap${checkAllId}`)
      .classList.add("border-checkbox");
  }
    ...
    for (i = 0; i < list.length; i++) {
      ...
      noteString += `<div class="notes-cover flex-row">
         <div id="note${list[i].id}" class="note" data-id="${list[i].id}">
            ...
            <button id='btn_checkAll${
              list[i].id
            }' class='btn-checkAll btn-bg cursor'><i class="fa-solid fa-check avoid-clicks"></i></button>
               <div class="note-title-wrap">
                  <div class="flex-row flex-bet">
                     <span id="note_title" class="note_title pad10">${
                       list[i].title
                     }</span>
                  ...
                  <div class="note-content-cover">
                     <p id="note_content" class="note_content pad10">${
                       list[i].content
                     }</p>
                  </div>               
                  <div id="list_label${list[i].id}" class="list-label ${
        labelInNote ? "" : "hiden"
      }">
                  <p id="list_label_name${list[i].id}">${
        labelInNote ? labelInNote.name : ""
      }</p>
                  <button id="removeLabelBtn${
                    list[i].id
                  }" class="remove-label-btn button-icon cursor"><i class="fa-solid fa-xmark avoid-clicks"></i></button>
               ...
               <div class="note-icon flex-row flex-bet">
                  ...
                  <button id="btn_option_${
                    list[i].id
                  }" class="btn-note-option button-icon cursor font-sz15 hiden display-note-icon"><i class="fa-solid fa-ellipsis"></i></button>
               </div>
            </div>
         </div>
      </div>`;
...
const mainNotes = () => {
  ...
  myFetch(link, option)
    .then((data) => {
      ...
      clickOutside();
      document
        .getElementById("header_menu_icon")
        .addEventListener("click", handleMenuBtn);
      document
        .getElementById("sidebar_btn_note")
        .addEventListener("click", showAllNotes);
      document
        .getElementById("handle_label")
        .addEventListener("click", displayHandleLabelModal);
      document
        .getElementById("delete_note")
